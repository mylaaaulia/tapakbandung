...
export default function FormEditLocationScreen() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onValue(itemRef, (snapshot) => {
            ...
            if (data) {
                ...
                const fetchedAccuracy = data.accuracy || '';
                // Strip non-numeric characters to handle old format (e.g., "10.50 m")
                const numericAccuracy = String(fetchedAccuracy).replace(/[^0-9.]/g, '');
                setAccuracy(numericAccuracy);
                if (data.coordinates) {
                    const [lat, lon] = String(data.coordinates).split(',');
                    setLatitude(lat || '');
                    setLongitude(lon || '');
                }
            }
    ...
    const handleSave = () => {
        ...
        if (isEdit) {
            update(ref(db, `points/${id}`), newLocation)
                .then(() => {
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <Stack.Screen options={{ title: isEdit ? 'Edit Lokasi' : 'Tambah Lokasi Baru' }} />
                    ...
                    <TouchableOpacity style={styles.saveButton} onPress={handleSave}>
                        <Text style={styles.saveButtonText}>{isEdit ? 'Simpan Perubahan' : 'Simpan Lokasi Baru'}</Text>
                    </TouchableOpacity>
